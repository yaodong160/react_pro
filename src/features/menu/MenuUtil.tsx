...
export function filterRoutesToMenus(routes: RouteObject[]) {
  ...
  for (const route of sortedRoutes) {
    const hideInMenu = route.handle?.hideInMenu;
    const isHidden = hideInMenu || String(hideInMenu) === 'true';
    // 调试：打印所有路由的 handle 信息
    console.log('[filterRoutesToMenus] route:', { id: route.id, path: route.path, handle: route.handle, isHidden, hasChildren: !!(route.children && route.children.length) });
    if (route.path && !isHidden) {
...
export function getSelectKey(route: Router.Route) {
  ...
  const routeName = (hideInMenu || String(hideInMenu) === 'true' ? activeMenu : name) || name;
